...
##    <Returns>a &homalg; matrix of size <C>1 x Length( </C><A>monomials</A><C> )</C></Returns>
##    <Description>
##      Let <C>R := HomalgRing(</C><A>poly</A><C>)</C>. This method computes <C>M</C>
##      (with entries in the coefficients ring of <C>R</C>, yet still considered as elements of <C>R</C>)
##      such that <C>poly = M * HomalgMatrix( monomials, Length( monomials ), 1, R )</C>.
##    <Listing Type="Code"><![CDATA[
InstallMethod( CoefficientsWithGivenMonomials,
        "for a ring element and a list of monomials able to generate the ring element",
        [ IsHomalgRingElement, IsList ],
        
  function( poly, monomials )
    local R, coeffs, RP, eval, sparse_coeffs_with_monomials, sparse_coeffs, sparse_monomials, coeff, monomial, pos, i;
    
    R := HomalgRing( poly );
    
    if IsZero( poly ) then
        coeffs := HomalgZeroMatrix( 1, Length( monomials ), R );
        return coeffs;
    fi;
    ...
    if IsBound( RP!.CoefficientsWithGivenMonomials ) then
        ...
        eval := RP!.CoefficientsWithGivenMonomials( poly, monomials );
        
        coeffs := HomalgMatrixWithAttributes( [
                Eval, eval,
                NrRows, 1,
                NrColumns, Length( monomials ),
            ], R );
    ...
    if not ( IsBound( RP!.Coefficients ) or IsHomalgInternalRingRep( R ) ) then
        Error( "could neither find a procedure called CoefficientsWithGivenMonomials ",
               "nor a procedure called Coefficients in the homalgTable of the non-internal ring\n" );
    fi;
    
    sparse_coeffs_with_monomials := Coefficients( poly );
    sparse_coeffs := EntriesOfHomalgMatrix( sparse_coeffs_with_monomials );
    sparse_monomials := sparse_coeffs_with_monomials!.monomials;
    
    coeffs := ListWithIdenticalEntries( Length( monomials ), Zero( R ) );
    
    for i in [ 1 .. Length( sparse_coeffs ) ] do
        
        coeff := sparse_coeffs[i];
        monomial := sparse_monomials[i];
        
        pos := Position( monomials, monomial );
        
        if pos = fail then
            Error( "the polynomial is not generated by the given monomials\n" );
        fi;
    ...
    return HomalgMatrix( coeffs, 1, Length( monomials ), R );
