               ...
               BestBasis :=
                 function( arg )
                   ...
                   if nargs > 1 then
                       ...
                       rank_of_S := Int( homalgSendBlocking( [ U, " := HomalgVoidMatrix(", R, ");; ", V, " := HomalgVoidMatrix(", R, ");; ", S, " := BestBasis(", M, U, V, ");; RowRankOfMatrix(", S, ")" ], "need_output" ) );
                   else
                       ## compute S only:
                       rank_of_S := Int( homalgSendBlocking( [ S, " := BestBasis(", M, ");; RowRankOfMatrix(", S, ")" ], "need_output" ) );
                   fi;
