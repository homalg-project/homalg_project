               ...
               BestBasis :=
                 function( arg )
                   ...
                   if nargs > 1 then
                       ...
                       rank_of_S := Int( homalgSendBlocking( [ S, " := ", R, "[-1][BestBasis](", M, R, "[1],", U, V, "): `homalg/RankOfGauss`(", S, R, "[-1])" ], "need_output" ) );
                   else
                       ## compute S only:
                       rank_of_S := Int( homalgSendBlocking( [ S, " := ", R, "[-1][BestBasis](", M, R, "[1]): `homalg/RankOfGauss`(", S, R, "[-1])" ], "need_output" ) );
                   fi;
