...
SINGULAR_READY := "!$%&/(";
SINGULAR_READY_LENGTH := Length( SINGULAR_READY );
  ...
  while true do  # will be exited with break or return
      ...
      if l[1] <> fail then   # something on stdout
          ...
          if bytes > 0 then
              ...
              gotsomething := true;
              pos := PositionSublist(s.lines,SINGULAR_READY,pos-SINGULAR_READY_LENGTH+1);
                  # ........NEWNEWNEWNEWNEW
                  #        ^
                  #        pos
              if pos <> fail then 
                  ...
                  s.lines := Concatenation(s.lines{[1..pos-1]},
                                     s.lines{[pos+SINGULAR_READY_LENGTH+1..Length(s.lines)]});
              fi;
          else
