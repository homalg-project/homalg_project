...
InstallMethod(OrbifoldTriangulation,"constructor",
  [IsList,IsRecord,IsList], function(s,i,m)
...
InstallMethod(SimplicialSet,"constructor",
  [IsOrbifoldTriangulation,IsInt], function(ot,n)
...
InstallMethod(BoundaryOperator,"calculate i-th boundary",
  [IsInt,IsList,IsFunction], function(i,L,mu)
...
InstallMethod(CreateCohomologyMatrix,"in GAP matrix type",
              [IsOrbifoldTriangulation,IsSimplicialSet],
              function(ot,s)
                    ...
                    for p in [1..Length(S[k])] do
                      for i in [0..k] do
                        ...
                        if not ind=fail then
                          matrices[k][ind][p]:=matrices[k][ind][p]+(-1)^i;
                        fi;
                      od;
                    od;
                  fi;
...
InstallMethod(CreateHomologyMatrix,"in GAP matrix type",
              [IsOrbifoldTriangulation,IsSimplicialSet],
              function(ot,s)
                local d,S,matrices,k,m,p,i,ind;
                d:=Dimension(s);
                S:=s!.simplicial_set;
                matrices:=[];
                matrices[1]:=NullMat(Length(S[1]),1);
                for k in [2..Dimension(s)] do
                  if Length(S[k])=0 then
                    matrices[k]:=NullMat(1,Length(S[k-1]));
                  else
                    matrices[k]:=NullMat(Length(S[k]),Length(S[k-1]));
                    for p in [1..Length(S[k])] do
                      for i in [0..k] do
                        ind:=PositionSet(S[k-1],BoundaryOperator(i,S[k][p],ot!.mu));
                        if not ind=fail then
                          matrices[k][p][ind]:=matrices[k][p][ind]+(-1)^i;
                        fi;
                      od;
                    od;
                  fi;
...
InstallMethod(CreateExternalCohomologyMatrix,"in Sage matrix type",
              [IsOrbifoldTriangulation,IsSimplicialSet,IsHomalgExternalRingInSageRep],
              function(ot,s,R)
                local d,S,matrices,k,m,p,i,ind;
                d:=Dimension(s);
                S:=s!.simplicial_set;
                matrices:=[];
                matrices[1]:=HomalgInitialMatrix(1,Length(S[1]),R);
                for k in [2..Dimension(s)] do
                  if Length(S[k])=0 then
                    ...
                    for p in [1..Length(S[k])] do
                      for i in [0..k] do
                        ...
                        if not ind=fail then
                          ##matrices[k][ind][p]:=matrices[k][ind][p]+(-1)^i;
                          homalgSendBlocking( [ matrices[k], "[", ind-1, ",", p-1, "]=", matrices[k], "[", ind-1, ",", p-1, "] + ", (-1)^i ], "need_command" );
                        fi;
                      od;
                    od;
                  fi;
...
InstallMethod(CreateExternalHomologyMatrix,"in Sage matrix type",
              [IsOrbifoldTriangulation,IsSimplicialSet,IsHomalgExternalRingInSageRep],
              function(ot,s,R)
                local d,S,matrices,k,m,p,i,ind;
                d:=Dimension(s);
                S:=s!.simplicial_set;
                matrices:=[];
                matrices[1]:=HomalgInitialMatrix(Length(S[1]),1,R);
                for k in [2..Dimension(s)] do
                  if Length(S[k])=0 then
                    ...
                    for p in [1..Length(S[k])] do
                      for i in [0..k] do
                        ...
                        if not ind=fail then
                          ##matrices[k][p][ind]:=matrices[k][p][ind]+(-1)^i;
                          homalgSendBlocking( [ matrices[k], "[", p-1, ",", ind-1, "]=", matrices[k], "[", p-1, ",", ind-1, "] + ", (-1)^i ], "need_command" );
                        fi;
                      od;
                    od;
                  fi;
...
InstallMethod(CreateExternalCohomologyMatrix,"in MAGMA matrix type",
              [IsOrbifoldTriangulation,IsSimplicialSet,IsHomalgExternalRingInMAGMARep],
              function(ot,s,R)
                local d,S,matrices,k,m,p,i,ind;
                d:=Dimension(s);
                S:=s!.simplicial_set;
                  ...
                  if Length(S[k])=0 then
                    ...
                    for p in [1..Length(S[k])] do
                      for i in [0..k] do
                        ...
                        if not ind=fail then
                          ##matrices[k][ind][p]:=matrices[k][ind][p]+(-1)^i;
                          homalgSendBlocking( [ matrices[k], "[", ind, p, "]:=", matrices[k], "[", ind, p, "] + ", (-1)^i ], "need_command" );
                        fi;
                      od;
                    od;
                  fi;
...
InstallMethod(CreateExternalHomologyMatrix,"in MAGMA matrix type",
              [IsOrbifoldTriangulation,IsSimplicialSet,IsHomalgExternalRingInMAGMARep],
              function(ot,s,R)
                local d,S,matrices,k,m,p,i,ind;
                d:=Dimension(s);
                  ...
                  if Length(S[k])=0 then
                    ...
                    for p in [1..Length(S[k])] do
                      for i in [0..k] do
                        ...
                        if not ind=fail then
                          ##matrices[k][p][ind]:=matrices[k][p][ind]+(-1)^i;
                          homalgSendBlocking( [ matrices[k], "[", p, ind, "]:=", matrices[k], "[", p, ind, "] + ", (-1)^i ], "need_command" );
                        fi;
                      od;
                    od;
                  fi;
