  ...
  while true do  # will be exited with break or return
      ...
      if l[1] <> fail then   # something on stdout
          ...
          if bytes > 0 then
              ...
              Error("Maple9 process seems to have died!");
          fi;
      fi;
...
SendMaple9Blocking := function(s,command)
  local l,nr;
  SendMaple9(s,command);
  repeat
      l := [IO_GetFD(s.stdout),IO_GetFD(s.stderr)];
      nr := IO_select(l,[],[],fail,fail);   # wait for input ready!
  until CheckMaple9Output(s) = true;
end;

LaunchMaple9 := function(arg)
  local s;
  s := IO_Popen3(Filename(DirectoriesSystemPrograms(),"maple9"),
                 [ "-q" ]);
  if s = fail then
      Error("No maple9 executable available");
  fi;
  s.stdout!.rbufsize := false;   # switch off buffering
  s.stderr!.rbufsize := false;   # switch off buffering
  SendMaple9Blocking(s,"\n");
