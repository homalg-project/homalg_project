...
SendMaple95 := function(s,command)
  local cmd;
  ...
  while true do  # will be exited with break or return
      ...
      if l[1] <> fail then   # something on stdout
          ...
          if bytes > 0 then
              ...
              Error("Maple95 process seems to have died!");
          fi;
      fi;
...
SendMaple95Blocking := function(s,command)
  local l,nr;
  SendMaple95(s,command);
  repeat
      l := [IO_GetFD(s.stdout),IO_GetFD(s.stderr)];
      nr := IO_select(l,[],[],fail,fail);   # wait for input ready!
  until CheckMaple95Output(s) = true;
end;

LaunchMaple95 := function(arg)
  local s;
  s := IO_Popen3(Filename(DirectoriesSystemPrograms(),"maple95"),
                 [ "-q" ]);
  if s = fail then
      Error("No maple95 executable available");
  fi;
  s.stdout!.rbufsize := false;   # switch off buffering
  s.stderr!.rbufsize := false;   # switch off buffering
  SendMaple95Blocking(s,"\n");
